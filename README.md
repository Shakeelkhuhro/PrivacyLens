...
flowchart TD
  subgraph Frontend [Expo React Native App]
    A1[Search or App List UI] --> A2[App Details Screen]
    A2 -->|GET /api/app/:query| B[Backend API]
    A3[Privacy Dashboard] --> A2
  end

  subgraph Backend [Express API]
    B --> B1[Resolve App ID]
    B1 --> B2[Scrape Play Store - axios, cheerio, google-play-scraper]
    B2 --> B3{Found privacyPolicyUrl?}
    B3 -->|yes| B4[Fetch privacy policy URL - axios]
    B3 -->|no| B5[Use Play Store text or developer website]
    B4 --> C[Extract policy text - cheerio to plain text]
    B5 --> C

    C --> G[Calculate privacy score - heuristics]
    G --> H[Respond JSON to Frontend including metadata.privacyScore]
    
    C --> D[LLM Enrichment - Azure OpenAI]
    D --> E[Normalize LLM JSON to fields: dataCollected, dataShared, securityPractices, summary]
    E --> F[Cache LLM result - node-cache TTL 24h & update dataSafety]
    
    E -->G2[re-calc privacy score & update cache]
    G2 --> F
  end

  subgraph LLM_Service [Azure OpenAI]
    D -->|API call| LLM[Azure OpenAI Deployment]
  end

  subgraph Infra
    B -. rate-limit .-> RL[express-rate-limit]
    B -. cors .-> CORS[Enable CORS]
  end

  style Frontend fill:#f0f8ff,stroke:#0366d6
  style Backend fill:#fff7e6,stroke:#9a6700
  style LLM_Service fill:#e8f5ff,stroke:#0078d4
  style Infra fill:#f6f6f6,stroke:#666

  classDef note fill:#fff8dc,stroke:#b8860b;
  subgraph Notes[ ]
    N1([Note: Score is computed from metadata + heuristic parsing of policy text.]):::note
    N2([Azure/OpenAI is optional - used to improve summaries and field extraction. Initial score does NOT depend on Azure.]):::note
  end
